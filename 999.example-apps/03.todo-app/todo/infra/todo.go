...
func (t *TodoRepo) Create(ctx context.Context, todos []model.Todo) error {
	...
	for _, mTodo := range todos {
		...
		_, err := conn.Exec(ctx, query, todo.Title, todo.Description, todo.Status, todo.Priority)
		if err != nil {
			return err
		}
	}
...
func (t *TodoRepo) ChangeStatus(ctx context.Context, todoID model.TodoID, status value.TodoStatus) error {
	...
	if _, err := conn.Exec(ctx, query, status, todoID); err != nil {
		return err
	}
...
func (t *TodoRepo) ChangePriority(ctx context.Context, todoID model.TodoID, priority value.TodoPriority) error {
	...
	if _, err := conn.Exec(ctx, query, priority, todoID); err != nil {
		return err
	}
	return nil
}
...
func (t *TodoRepo) ChangeTag(ctx context.Context, todoID model.TodoID, tag model.Tag) error {
	...
	tagQuery := sb.String()
	var tagID int64
	if err := conn.QueryRow(ctx, tagQuery, tag.Name).Scan(&tagID); err != nil {
		return err
	}
	...
	query := sb.String()
	_, err = conn.Exec(ctx, query, todoID, tagID)
	if err != nil {
		return err
	}
...
func (t *TodoRepo) GetByPriorityAndStatus(ctx context.Context, priority value.TodoPriority, status value.TodoStatus) ([]model.Todo, error) {
	...
	rows, err := conn.Query(ctx, query, priority, status)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		modelTodo := model.Todo{
			...
			Priority: func() value.TodoPriority {
				priority, err := value.TodoPriorityString(todo.Priority)
				if err != nil {
					return value.LOW
				}
				return priority
